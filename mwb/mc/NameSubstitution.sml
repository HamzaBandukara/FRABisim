...
(*  exception not_yet_implemented *)
  ...
  fun if_bound t x nil = (* raise bound_name_expected(x,x) *) false |
      if_bound t x (l::r) =
  ...
  fun is_eq x y nil = (* raise bound_name_expected(x,y) *) false |
      is_eq x y (l::r) =
         if McList.member N.curry_eq x l
  ...
  fun is_neq x y nil = (* raise bound_name_expected(x,y) *) false |
      is_neq x y (l::r) =
         if McList.member N.curry_eq x l
         then if McList.member N.curry_eq y l
              then false
...
**** *)
  ...
  fun add_distinct x nc =
      let val _ = if !Flags.tracelevel > 1 then
		  print("*add_distinct "^(N.mkstr x)^" "^(mkstr nc)^"\n")
		  else ()
  ...
  fun add_new x nc =
	  ...
	  val _ = if !Flags.tracelevel > 1 then
		  print("*add_new "^(N.mkstr x)^" "^(mkstr nc)^"\n")
		  else ()
  ...
  fun partition1 nil c = [c] |
      partition1 (x::nl) c =
	    ...
	    let val _ = if !Flags.tracelevel > 2 then
			print("*partition1 ["^(Lib.mapconcat N.mkstr (x::nl) ",")^"] "^(mkstr c)^"\n") else ()
  ...
  fun entails ns b =
      B.implies (Lib.fold (fn (s,t) =>
		       B.join(t,(Lib.fold (fn (x,r) =>
...
(*      raise not_yet_implemented *)
  ...
  fun private x nil = [[x]] | (* BV, LHE replaced nil *)
      private x (nl::ns) =
        case McList.l_rm_and_tell N.curry_eq x nl of
          (true,nil) => [x]::ns |
          (true,nl1) => let val _=print("PRIVATE ALERT!\n") in [x]::nl1::ns end |
