...
(*   fun mk_diamond (ACT.mk_act x) F = Diamond(unbarred,bind_list [x] F) | *)
(*       mk_diamond (ACT.mk_bar x) F = Diamond(barred,bind_list [x] F) |   *)
(*       mk_diamond (ACT.tau ()) F = Diamond(invisible,F)                  *)
  fun mk_diamond x F =
...
(*   fun mk_box (ACT.mk_act x) F = Box(unbarred,bind_list [x] F) | *)
(*       mk_box (ACT.mk_bar x) F = Box(barred,bind_list [x] F) |   *)
(*       mk_box (ACT.tau ()) F = Box(invisible,F)                  *)
  fun mk_box x F =
      if ACT.is_input(x) then
  ...
  fun mk_not (True) = False |
      ...
      mk_not (Pi(F)) = Exists(mk_not F) |
      mk_not (Exists(F)) = Pi(mk_not F) |
      mk_not (Box(M,F)) = Diamond(M,mk_not(F)) |
      mk_not (Diamond(M,F)) = Box(M,mk_not(F)) |
      mk_not (F as Sigma(_)) = 
         raise NegationError("Presently cannot negate Sigma", F) |
      mk_not (F as BSigma(_)) = 
         raise NegationError("Presently cannot negate Bsigma", F) |
      mk_not (F as RootedVar(p,vl)) =
         raise NegationError("Fixpoint name occurence must have same polarity as the fixpoint expression", F) |
      mk_not (F as RootedCon(c,vl)) =
         raise NegationError("Fixpoint name occurence must have same polarity as the fixpoint expression", F) |
      mk_not (F as RootedGFP(p,n,f,vl)) =
         raise NegationError("Presently cannot negate fixpoints", F) |
      mk_not (F as RootedLFP(p,n,f,vl)) =
         raise NegationError("Presently cannot negate fixpoints", F)
...
(*   fun get_body (GFP(X,n,F)) nl = instantiate_list 1 nl F | *)
(*       get_body (LFP(X,n,F)) nl = instantiate_list 1 nl F   *)
