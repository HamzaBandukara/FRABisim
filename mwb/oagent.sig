...
sig
   ...
   val ceq : C.cond -> agent * agent -> bool
   ...
   val mk_match : C.cond * agent -> agent
(*    val mk_conditional : C.cond * agent * agent -> agent *)
...
(*    val is_conditional : agent * agent E.env -> bool *)
   ...
   val match_test : agent * agent E.env -> C.cond
   val match_positive : agent * agent E.env -> agent
(*    val conditional_test : agent * agent E.env -> C.cond   *)
...
(*    val conditional_negative : agent * agent E.env -> agent *)
   ...
   val free_names : agent -> C.N.name list
   val names : agent * agent E.env -> C.N.name list

   val apply : agent * (C.N.name list) * agent E.env -> agent
   val beta_reduce : agent -> (C.N.name list * int) -> agent
   val std_form : agent * agent E.env -> agent

   val substitute : (C.N.name * C.N.name) list * agent -> agent

   (* for debug? *)
   val abs_body : agent * int * agent E.env -> agent
   val abs_all : agent * agent E.env -> (int * agent)
   val conc_all : agent * agent E.env -> (int * C.N.name list * agent)
