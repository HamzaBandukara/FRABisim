...
class Process:
                ...
                if s and f(k,s) and len(opts)>1:
                    for nmap, acc in opts:
                        ...
                        for t in acc.strArray():
                            print("  " + t)
                        print("}")
                    raise("more than one transition sets",len(opts))
    ...
    def step(self, strict=True):
        ...
        if (self, strict) in Process.lts:
            debprint("memo",self,strict,self.nform)
            opts = Process.lts[(self, strict)]
            for nmap, acc2 in opts:
                if issubmap(self.nmap,nmap):
                    if (not strict) or issubmap(nmap,self.nmap):
                        return acc2
                    continue
                if strict: continue # nmap mismatch
                if issubmap(nmap,self.nmap): 
                    acc = TransitionSet()
                    debprint("FOOR",self,strict)            
                    for elem in acc2:
                        p = elem[1].copyUp(self.nmap)
                        acc.add((elem[0],p))
                        debprint("-> adding",elem[0],p,"from",elem[1])
                    Process.lts[(self,strict)].append((self.nmap,acc))
                    return acc
        acc = TransitionSet()  
        if strict and (self, False) in Process.lts:
            opts = Process.lts[(self, False)]
            for nmap, acc2 in opts:
                if issubmap(self.nmap,nmap):
                    debprint("FOR",self,strict)            
                    for elem in acc2:
                        p = elem[1].copyDown(elem[1].proc)
                        acc.add((elem[0],p))
                        debprint("-> adding",elem[0],p,"from",elem[1])
                    try: Process.lts[(self,strict)].append((self.nmap,acc))
                    except: Process.lts[(self,strict)] = [(self.nmap,acc)]
                    return acc
        # continue if not in memo, or what is in memo is not compatible
        ...
        if ty == "zero":
                        ...
                        for pLnext in theL[xL[iL]]:
                            for pRnext in theR[xR[iR]]:
                                if left:
                                    ...
                                    p = pRnext
                                p = p.copy(RawProcess("par", (pLnext.proc, pRnext.proc)))
                                acc.add(("_t", p))
                        iL += 1;
                        iR += 1
                # Case CloseL/R:
            ...
            try: Process.lts[(self,False)].append((self.nmap,acc))
            except: Process.lts[(self,False)] = [(self.nmap,acc)]
            debprint("for",self,False)
            for (x,y) in acc: debprint("=> adding",x,y)
            ...
            p.rehash()
            acc = p.step(False)  # TODO*: initial creation not needed            
            # we first calculate the non-strict step (why?)
            try: Process.lts[(self,False)].append((self.nmap,acc))
            except: Process.lts[(self,False)] = [(self.nmap,acc)]
            debprint("for",self,False)
            for (x,y) in acc: debprint("=> adding",x,y)
        ...
        debprint("for",self,strict)
        for e in acc: debprint("-> adding",e[0],e[1])
        return acc
...
if __name__ == '__main__':
    ...
    Q0= RawProcess("var",(U[0],[n[0]]))
    P = RawProcess("par",(P0,Q0))
    P = Process({0}, {n[0]: 0}, P)
    print(P)
    print(P.nform)
    # print(P.proc.nid)
