...
def record(path, file, representation, q1, s, q2, repetitions=3):
    ...
    with open("paper_times/stacks_1.csv", "a") as f:
...
def benchmarks(size=30, repetitions=1, start=1, steps=1):
    if not os.path.exists("../Benchmarks"):
        os.mkdir("../Benchmarks")
    output_file = open("paper_times/stacks_1.csv", "w")
    output_file.write("TestCode,RA,States,Transitions,Memo,Exception,Generator,PRT-REF,Result,DEQ-Result,DEQ-Time,LOIS FW Result,LOIS FW Time,LOIS PR Result,LOIS PR Time\n")
    ...
    for i in range(start, size, steps):
        ...
        record("Test_01_B", "{}".format(i), representation, q1, set(), q1, repetitions)
        output_file = open("paper_times/stacks_1.csv", "a")
...
if __name__ == '__main__':
    ...
    # deq(None, None)
    benchmarks(size=15, repetitions=3, start=1, steps=1)
