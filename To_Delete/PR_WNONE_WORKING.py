...
def bisim_ok(q1, sigma: PartialPermutation, q2, partition_dict):
    ...
    for s1 in partition_dict[q1]:
        for s2 in partition_dict[q2]:
            if partition_dict[q1][s1] == partition_dict[q2][s2]:
                p = PartialPermutation(sigma.full_domain, set(zip(s1, s2)))
                if sigma <= p:
                    return True
    return False
