    ...
    def normal(self):
        ...
        for i in range(len(nproc)):
            if isinstance(nproc[i],Name):
                if nproc[i] in self.nmap:
                    i2 = self.nmap[nproc[i]]
                    iset2 -= {i2}
                    nproc[i]=str(i2)
                else: nproc[i] = str(nproc[i])
        if len(iset2)==0:
            return "".join(nproc)
        iset2 = list(iset2); iset2.sort()
    ...
    def step(self):
        ...
        if ty=="zero":
            ...
            for _ in range(2):
                ...
                for elem in new_acc:
                    ...
                    if elem[0] == "_t" or elem[0][0]=="outK":
                        ...
                        if elem[0][0] == "inpK":
                            n2 = pnext.newFreeOf(self.proc)
                        else:
                            n2 = elem[1].nmapInv(i2) # TODO*: this could be done as above?
                        if n2 is not None:
                            if elem[0][0] != "inpK":
                                i2 = p.minIempty()
                            p.substitute(i2, n2)
            ...
            for t in new_acc:
                flg = True
                if t[0][0] == "outK":
                    if t[0][2] == p.nmap[n1] and t[0][2] != t[0][1]:
                        # n1i = t[1].minIempty()
                        # t[1].nmap[n1] = n1i
                        # t[1].rehash()
                        acc.add((("outF", t[0][1], t[0][2]), t[1]))
                    else:
                        flg = False
                else: flg = False
                if not flg:
